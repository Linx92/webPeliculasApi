    ...
    public class PeliculasController:CustomBaseController
    {
        ...
        public async Task<ActionResult<List<PeliculaDTO>>> Filtrar([FromQuery] FiltroPeliculaDTO filtroPeliculaDTO) 
        {
            ...
            if (filtroPeliculaDTO.GeneroId != 0)
            {
                peliculasQueryable = peliculasQueryable
                    .Where(x => x.PeliculasGeneros.Select(y => y.GeneroId).Contains(filtroPeliculaDTO.GeneroId));
            }
        ...
        public async Task<ActionResult<PeliculaDetallesDTO>> Get(int id) 
        {
            var peliculaBD = await context.Peliculas
                ...
                .ThenInclude(x=>x.Genero).FirstOrDefaultAsync(x => x.Id == id);
            if (peliculaBD == null) 
            {
                return NotFound();
            }
            peliculaBD.PeliculasActores = peliculaBD.PeliculasActores.OrderBy(x=>x.Orden).ToList();
            return mapper.Map<PeliculaDetallesDTO>(peliculaBD);
        }
        [HttpPost]
        public async Task<ActionResult> Post([FromForm]PeliculaCreacionDTO peliculaCreacionDTO) 
        {
            var peliculaDB = mapper.Map<Pelicula>(peliculaCreacionDTO);
